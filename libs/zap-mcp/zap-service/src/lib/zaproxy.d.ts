declare module 'zaproxy' {
  ...
  export interface ZAPResponse<T = any> {
    [key: string]: T;
  }
  ...
  export class ZAPClient {
    ...
    context: {
      ...
      context(contextName: string): Promise<{ context: any }>;
      setContextInScope(
        contextName: string,
      ...
      setContextCheckEveryResponse(
        contextName: string,
        booleanCheckEveryResponse: boolean
      ): Promise<ZAPResponse>;
    };

    authentication: {
      getSupportedAuthenticationMethods(): Promise<{
        supportedMethods: string[];
      }>;
      getAuthenticationMethodConfigParams(
        authMethodName: string
      ): Promise<{ methodConfigParams: string[] }>;
      getAuthenticationMethod(contextId: string): Promise<{ method: any }>;
      getLoggedInIndicator(
        contextId: string
      ): Promise<{ loggedInIndicator: string }>;
      getLoggedOutIndicator(
        contextId: string
      ): Promise<{ loggedOutIndicator: string }>;
    ...
    users: {
      usersList(contextId?: string): Promise<{ usersList: any[] }>;
      getUserById(contextId: string, userId: string): Promise<{ user: any }>;
      getAuthenticationCredentialsConfigParams(
        contextId: string
      ): Promise<{ credentialsConfigParams: string[] }>;
    ...
    sessionManagement: {
      getSupportedSessionManagementMethods(): Promise<{
        supportedMethods: string[];
      }>;
      getSessionManagementMethodConfigParams(
        methodName: string
      ): Promise<{ methodConfigParams: string[] }>;
      getSessionManagementMethod(contextId: string): Promise<{ method: any }>;
    ...
    spider: {
      ...
      fullResults(scanId: string): Promise<{ fullResults: any[] }>;
      scans(): Promise<{ scans: any[] }>;
      excludedFromScan(): Promise<{ excludedFromScan: string[] }>;
      allUrls(): Promise<{ urls: string[] }>;
      addedNodes(scanId?: string): Promise<{ addedNodes: any[] }>;
      ...
      optionHandleODataParametersVisited(): Promise<{
    ...
    ascan: {
      ...
      scanners(
        ...
        policyId?: string
      ): Promise<{ scanners: any[] }>;
      policies(
        scanPolicyName?: string,
        policyId?: string
      ): Promise<{ policies: any[] }>;
      attackModeQueue(): Promise<{ attackModeQueue: number }>;
      excludedParams(): Promise<{ excludedParams: any[] }>;
      excludedParamTypes(): Promise<{ excludedParamTypes: any[] }>;
      ...
      optionMaxScansInUI(): Promise<{ MaxScansInUI: number }>;
      optionTargetParamsEnabledRPC(): Promise<{
        TargetParamsEnabledRPC: boolean;
      }>;
      optionTargetParamsInjectable(): Promise<{
        TargetParamsInjectable: number;
      }>;
      optionThreadPerHost(): Promise<{ ThreadPerHost: number }>;
      optionAllowAttackOnStart(): Promise<{ AllowAttackOnStart: boolean }>;
      optionInjectPluginIdInHeader(): Promise<{
        InjectPluginIdInHeader: boolean;
      }>;
      optionPromptInAttackMode(): Promise<{ PromptInAttackMode: boolean }>;
      optionPromptToClearFinishedScans(): Promise<{
        PromptToClearFinishedScans: boolean;
      }>;
      optionRescanInAttackMode(): Promise<{ RescanInAttackMode: boolean }>;
      optionScanHeadersAllRequests(): Promise<{
    ...
    httpSessions: {
      sessions(site: string, session?: string): Promise<{ sessions: any[] }>;
    ...
    autoupdate: {
      ...
      installedAddons(): Promise<{ installedAddons: any[] }>;
      newAddons(): Promise<{ newAddons: any[] }>;
      updatedAddons(): Promise<{ updatedAddons: any[] }>;
      marketplaceAddons(): Promise<{ marketplaceAddons: any[] }>;
    ...
    // Additional APIs can be added here following the same pattern:
    // - params
    // - pnh
    // - alertFilter
    // - authorization
